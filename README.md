...
Contact::Contact(string first, string last, string mobile, string email, Address*
add)
{
if (!first.empty()) {
first_name = first;
...
Contact* Contact::copy_contact() {Contact* newContact = new Contact;
...
public:
Address();
Address(string h, string s, string ci, string c);string getHouse();
string getStreet();
string getCity();
string getCountry();
...
{
cout << "House No. " << house << ", Street " << street << ", " << city << ",
" << country;
}
...
string email_address;Address* address;
public:
Contact();
Contact(string first, string last, string mobile, string email, Address*
add);
...
cout << "Contact added to group '" << groupName << "' successfully." <<
endl;
}
else {
...
delete groupContacts[num - 1]for (int i = num - 1; i < size - 1; i++) {
groupContacts[i] = groupContacts[i + 1];
}
groupContacts[size - 1] = nullptr; // Update the last element to nullptr
size--;
cout << "Contact removed from group '" << groupName << "' successfully." <<
endl;
}
else {
...
if (choice == 1)
{
if (contacts_list[j]->getFirstName() > contacts_list[j +
1]->getFirstName())
{
//swap contacts
...
{
if (contacts_list[j]->getLastName() > contacts_list[j +
1]->getLastName())
{
// swap contacts
Contact* temp = contacts_list[j];
...
for (int i = 0; i < contacts_count; i++)
{
if (contacts_list[i]->getFirstName() == general || contacts_list[i]->getLastName()
== general || contacts_list[i]->getEmailAddress() == general ||
contacts_list[i]->getMobileNumber() == general)
{
return new Contact(*(contacts_list[i]));
}
else if (contacts_list[i]->getAddress()->getHouse() == general ||
contacts_list[i]->getAddress()->getStreet() == general || contacts_list[i]->getAddress()->getCity()
== general || contacts_list[i]->getAddress()->getCountry() == general)
{
return new Contact(*(contacts_list[i]));
}
}
...
{
cout << "\nA duplicate contact has been found and both are
merged\n";
delete contacts_list[j];
flag = true;//to fill up the space
...
for (int i = 0; i < contacts_count; i++) {
outfile << "Contact " << i + 1 << ":\n";
outfile << "Name: " << contacts_list[i]->getFirstName() << " " <<
contacts_list[i]->getLastName() << endl;
outfile << "Mobile Number: " << contacts_list[i]->getMobileNumber() << endl;
outfile << "Address: House No. " << contacts_list[i]->getAddress()->getHouse()
<< ", Street ";
...
cout << "Country: " << contacts_list[n]->getAddress()->getCountry() <<
endl;
...
cout << "City: " << contacts_list[n]->getAddress()->getCity() << endl;
cout << "Country: " << contacts_list[n]->getAddress()->getCountry() <<
endl;// cout << "Group: " << contacts_list[n]->getGroup() << endl << endl;
cout << endl;
contact_count++;
}
}
...
cout << "\nMaximum number of groups reached. Cannot create more groups." <<
endl;
}
}
...
{
if (historyCount < 100)if (historyCount < 100)
{
...
int SearchHistory::historyCount = 0CONTACTBOOK.H
#pragma once
#include "Address.h"
#include "Contact.h"
#ifndef BASIC_LIB
...
contactbook.updateContact(delContChoice,
takeInputs(tempContact));
cout << endl;
system("pause");
system("cls");
continue;
...
{
contactbook.search_contact(first,
last)->printContactInfo();
}
cout << endl;
system("pause");
system("cls");
...
{
cout << "Enter mobile number of the person you want to
find: ";
cin.ignore();
getline(cin, mobile);
history.SearchBox(mobile)cout << endl;
...
string combineAddress = "House No. " + house + ", Street "
+ street + ", " + city + ", " + country;
...
{
cout << "\nTotal number of contacts saved in contact book = " <<
contactbook.total_contacts() << endl;
cout << endl;
system("pause");
system("cls");
}
...
{
cout << "Enter name of group you want to add
contacts in: ";
cin.ignore();
getline(cin, first);
cout << "\nEnter details of contact you want to
add:\n";
